...
const FAQ = () => {
  return (
    ...
    >
      {/* Left Section (Text) */}
      <div className="flex flex-col items-start text-left basis-full md:basis-1/2 max-w-7xl">
        ...
        <Accordion type="single" collapsible className="w-full">
          {FrequentlyAskedQuestions.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger className="text-base md:text-xl font-bold text-zinc-800 dark:text-white active:text-green-500 py-4 px-2 md:px-4 text-left">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="text-sm md:text-base font-semibold text-zinc-600 dark:text-zinc-400 whitespace-pre-line px-2 md:px-4 text-left">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
