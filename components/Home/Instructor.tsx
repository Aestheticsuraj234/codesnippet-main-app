...
export const AnimatedFounders = ({
  ...
  const handleNext = () => {
    setActive((prev) => (prev + 1) % Founderss.length);
  };

  const handlePrev = () => {
    setActive((prev) => (prev - 1 + Founderss.length) % Founderss.length);
  };
        ...
        <div className="flex justify-between flex-col py-4">
          ...
          >
            <h3 className="text-2xl font-bold dark:text-white text-black">
              {Founderss[active].name}
            </h3>
            <p className="text-sm text-gray-500 dark:text-neutral-500">
              {Founderss[active].designation}
            </p>
            <motion.p className="text-lg text-gray-500 mt-8 dark:text-neutral-300">
              {Founderss[active].quote.split(" ").map((word, index) => (
                <motion.span
                  key={index}
                  initial={{
                    filter: "blur(10px)",
