...
import { toggleMarkAsDone } from "@/action/live-course";
...
const ActionTabMenu = ({ isMarkAsDone, userId, chapterId }: ActionMenuProps) => {
  ...
  const onMarkAsDone = async () => {
    ...
    try {
      await toggleMarkAsDone(chapterId, userId, newCompletedStatus);
    } catch (error) {
