...
import { AddDayAssingToTopic, ADDNumberOfDaysInTopic } from "@/action/tutorial";
...
export const AddModal = () => {
  ...
  const isModalOpen = isOpen && type === "ADD_DAY";
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const res = await ADDNumberOfDaysInTopic(data.topicId, values.numberOfDays);
      form.reset();
      ...
      <DialogContent className="bg-white text-black p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500">
            Add days to your topic to start your journey 
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          {/* @ts-ignore */}
