        ...
        {mounted &&
          Object.entries(leaderboardData).map(([key, users]) => (
            <TabsContent key={key} value={key} className="space-y-4 p-4">
              {users.length === 0 ? (
                <EmptyStateComponent
                  imageUrl="/empty-points-ca.svg" // Replace with actual image path
                  title={`No ${key.charAt(0).toUpperCase() + key.slice(1)} Points Found`}
                  description={`There are currently no ${key} Points to display. Please check back later.`}
                />
              ) : (
